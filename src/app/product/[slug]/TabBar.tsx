...
export default function TabBar({ reviewsComponent, descriptionComponent }: TabBarProps) {
  const [value, setValue] = useState<TabItemsType>('Description');

  const handleChange = (event: React.SyntheticEvent, newValue: TabItemsType) => {
    setValue(newValue);
  };
  ...
  return (
    <>
      <Box sx={{ borderBottom: 3, borderColor: 'divider', marginBottom: '6px' }}>
        <Tabs value={value} onChange={handleChange} aria-label="Product Detail Navigation">
